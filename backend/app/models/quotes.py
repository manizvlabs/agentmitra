...
class DailyQuote(Base, TimestampMixin, AuditMixin):
    ...
    agent_id = Column(UUID(as_uuid=True), ForeignKey("lic_schema.users.user_id", ondelete="CASCADE"), nullable=False)
    ...
    sharing_analytics = relationship("QuoteSharingAnalytics", back_populates="quote")
    performance_metrics = relationship("QuotePerformance", back_populates="quote")
...
class QuoteSharingAnalytics(Base, TimestampMixin):
    ...
    quote_id = Column(UUID(as_uuid=True), ForeignKey("lic_schema.daily_quotes.quote_id", ondelete="CASCADE"), nullable=False)
    agent_id = Column(UUID(as_uuid=True), ForeignKey("lic_schema.users.user_id", ondelete="CASCADE"), nullable=False)
